...
namespace cmangos_module
{
...
#ifdef ENABLE_PLAYERBOTS
        if (!m_module->GetConfig()->randomBots && !m_player->isRealPlayer())
            return;
#endif
...
#ifdef ENABLE_PLAYERBOTS
            if (!m_module->GetConfig()->randomBotsRealmFirst && !m_player->isRealPlayer())
                return false;
#endif
        }
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(m_completedAchievementsMutex);
    ...
    void AchievementsModule::LoadAchievementCategories()
    {
        ...
        if (result)
        {
            ...
            do
            {
                ...
                category->name[14] = fields[16].GetCppString();
    ...
    void AchievementsModule::LoadCompletedAchievements()
    {
        ...
        // Pre-populate realm-first keys; access to the map is serialized by
        // m_completedAchievementsMutex for multi-map-update builds.
        for (const auto& pair : m_achievements)
        {
            const AchievementEntry* achievement = &pair.second;
    ...
    void AchievementsModule::OnPreCharacterCreated(Player* player)
    {
        if (GetConfig()->enabled)
        {
            if (player)
            {
                ...
                if (!GetConfig()->randomBots && !player->isRealPlayer())
                    return;
    #endif
                // Create the player achievement manager
                std::lock_guard<std::recursive_mutex> lock(m_playerMgrsMutex);
