...
namespace cmangos_module
{
    ...
    bool TransmogModule::HandleCalculateTransmogCost(WorldSession* session, const std::string& args)
    {
        ...
        if (GetConfig()->enabled)
        {
            ...
            if (player)
            {
                ...
                if (!transmog_detail::ParseRequest(args, slots) || !ValidateRequest(player, slots))
                {
                    SendAddOnMessage(player, GetChatCommandPrefix(), "TransmogCost:0:0:0");
                    return true;
                }
        ...
        if (GetConfig()->enabled)
        {
            ...
            if (player)
            {
                ...
                if (slots.size() > 0)
                {
                    ...
                    succeeded = cost <= uint64(std::numeric_limits<int32>::max()) && (tokenID ? player->HasItemCount(tokenID, cost) : player->GetMoney() >= cost);
                }
                ...
                if (succeeded)
                {
                    if (tokenID)
                    {
                        player->DestroyItemCount(tokenID, uint32(cost), true);
                    }
                    else
                    {
    ...
    bool TransmogModule::CanUseTransmogNpc(Player* player) const
    {
        if (!player || !player->IsAlive() || player->IsInCombat()) return false;
        Creature* npc = player->GetNPCIfCanInteractWith(player->GetSelectionGuid(), UNIT_NPC_FLAG_GOSSIP);
        return npc && npc->GetEntry() == 190010;
    }

    bool TransmogModule::ValidateRequest(const Player* player, const std::vector<std::pair<uint32, uint32>>& slots) const
    {
        if (!player || slots.empty()) return false;
        auto discovered = playerDiscoveredTransmogs.find(player->GetGUIDLow());
        for (const auto& pair : slots)
        {
            if (pair.first >= EQUIPMENT_SLOT_END) return false;
            Item* target = player->GetItemByPos(INVENTORY_SLOT_BAG_0, uint8(pair.first));
            if (!target || !target->IsEquipped()) return false;
            if (!pair.second) continue; // Restore the equipped item's own appearance.
            const ItemPrototype* source = sObjectMgr.GetItemPrototype(pair.second);
            if (!source || !IsValidTransmog(player, source) || discovered == playerDiscoveredTransmogs.end()) return false;
            auto known = discovered->second.find(source->DisplayInfoID);
            if (known == discovered->second.end() || known->second.itemID != pair.second) return false;
            const ItemPrototype* dest = target->GetProto();
            auto inventoryType = [](uint32 type) { return type == INVTYPE_ROBE ? uint32(INVTYPE_CHEST) : type; };
            if (source->Class != dest->Class || source->SubClass != dest->SubClass ||
                inventoryType(source->InventoryType) != inventoryType(dest->InventoryType)) return false;
            const auto& allowed = known->second.slots;
            if (std::find(std::begin(allowed), std::end(allowed), pair.first) == std::end(allowed)) return false;
        }
        return true;
    }
    ...
    std::vector<std::pair<Item*, uint32>> TransmogModule::GetTransmogrifiedItems(const Player* player, bool equipped) const
    {
        ...
        if (player)
        {
            ...
            {
                auto playerTransmogsIt = entryMap.find(player->GetObjectGuid().GetCounter());
            ...
            if (result)
            {
                do
                {
                    ...
                    else
                    {
                        ...
                        CharacterDatabase.PExecute("DELETE FROM `custom_transmog_discovered` WHERE `player` = %u AND `item_entry` = %u", playerID, itemEntry);
                    }
                }
                while (result->NextRow());
            }
        ...
        if (it != playerDiscoveredTransmogs.end())
        {
            ...
            if (const ItemPrototype* proto = sObjectMgr.GetItemPrototype(itemEntry))
            {
                if (availableTransmogs.find(proto->DisplayInfoID) == availableTransmogs.end())
                {
                    ...
                    std::fill(std::begin(transmogItem.slots), std::end(transmogItem.slots), NULL_SLOT);
                    ...
                    if (player->ViableEquipSlots(proto, &transmogItem.slots[0]))
                    {
                        if (addToDB)
                        {
                            CharacterDatabase.PExecute("INSERT IGNORE INTO `custom_transmog_discovered` (`player`, `item_entry`) VALUES (%u, %u)", playerID, transmogItem.itemID);
                        }
    ...
    std::pair<uint32, uint32> TransmogModule::CalculateTransmogCost(uint32 itemEntry) const
    {
        ...
        else
        {
            if (const ItemPrototype* proto = sObjectMgr.GetItemPrototype(itemEntry))
            {
                const double price = (double(proto->SellPrice ? proto->SellPrice : 100U) +
                    GetConfig()->costFee) * GetConfig()->costMultiplier;
                cost = uint32(std::min<double>(price, std::numeric_limits<int32>::max()));
            }
        }
    ...
    void TransmogModule::SendTransmogCost(const Player* player, const std::vector<std::pair<uint32, uint32>>& slots) const
    {
        if (player)
        {
            ...
            if (slots.size() > 0)
            {
                ...
                canPurchase = cost <= uint64(std::numeric_limits<int32>::max()) && (tokenID ? player->HasItemCount(tokenID, cost) : player->GetMoney() >= cost);
            }

            SendAddOnMessage(player, GetChatCommandPrefix(), helper::FormatString
            (
                "TransmogCost:%u:%u:%u",
                uint32(std::min<uint64>(cost, std::numeric_limits<uint32>::max())),
                tokenID,
                canPurchase ? 1 : 0
            ));
        }
